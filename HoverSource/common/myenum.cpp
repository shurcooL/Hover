                                          ...
                                          VOID* pParentInfo )
{
    ...
    for( DWORD i=0; i<pDriverInfo->dwNumModes; i++ )
    {
			...
			DWORD dwZBufferDepths	= pDeviceInfo->ddDeviceDesc.dwDeviceZBufferBitDepth;
			DWORD dwStencilCaps		= pDeviceInfo->ddDeviceDesc.dwStencilCaps;
			DWORD dwDepth           = ddsdMode.ddpfPixelFormat.dwRGBBitCount;
			DWORD dwNumStencilBits	= GetNumberOfBits( ddsdMode.ddpfPixelFormat.dwStencilBitMask );
    ...
    for( i=0; i<pDeviceInfo->dwNumModes; i++ )
    {
			...
			{
				// D3DDEVICEDESC7 temp = pDeviceInfo->ddDeviceDesc;
...
//		if( FAILED( hr = pD3D->EnumZBufferFormats( *(d3dDeviceInfo.pDeviceGUID), 
//			EnumZBufferFormatsCallback, (VOID*)&blah ) ) ){
//			CHAR msg[200];
//			D3DXGetErrorString( hr, 200, &msg[0] );
//			MyAlert( msg );
//		}

//    pD3D->EnumZBufferFormats( (*m_pDeviceInfo->pDeviceGUID),
...
static VOID UpdateDialogControls( HWND hDlg, D3DEnum_DeviceInfo* pCurrentDevice,
                                  DWORD dwCurrentMode, BOOL bSimplerShadows )
{
    ...
    HWND hwndSimplerShadows	= GetDlgItem( hDlg, IDC_SIMPLERSHADOWS_CHECKBOX );

    // Reset the content in each of the combo boxes
    ComboBox_ResetContent( hwndDevice );
    ComboBox_ResetContent( hwndMode );

		Button_SetCheck( hwndSimplerShadows, bSimplerShadows );
    ...
    for( DWORD device = 0; device < dwNumDevices; device++ )
    {
        ...
        if( pDevice == pCurrentDevice )
        {
            ...
            EnableWindow( hwndMode, TRUE );
            EnableWindow( hwndFullscreenText, TRUE );
                                       ...
                                       LPARAM lParam )
{
    ...
    static DWORD dwCurrentMode;
		static BOOL* pbCurrentSimplerShadows;
    ...
    if( WM_INITDIALOG == uiMsg )
    {
        ...
        ppDeviceArg = ((TEMPSTRUCT *)lParam)->ppDeviceInfo;
        if( NULL == ppDeviceArg )
            return FALSE;
        ...
        dwCurrentMode    = pCurrentDevice->dwCurrentMode;
				...
				pbCurrentSimplerShadows = ((TEMPSTRUCT *)lParam)->pbSimplerShadows;

        UpdateDialogControls( hDlg, pCurrentDevice, dwCurrentMode, *pbCurrentSimplerShadows );
    ...
    else if( WM_COMMAND == uiMsg )
    {
        ...
        BOOL  bSimplerShadows = Button_GetCheck( hwndSimplerShadows );
        ...
        if( IDOK == LOWORD(wParam) )
        {
            ...
            if( pDevice != pCurrentDevice || dwMode != dwCurrentMode ||
							bSimplerShadows != *pbCurrentSimplerShadows )
            {
                ...
                pDevice->ddsdFullscreenMode = pDevice->pddsdModes[dwMode];
								...
								*pbCurrentSimplerShadows		= bSimplerShadows;
        ...
        // Keep the UI current
        UpdateDialogControls( hDlg, &pDeviceList[dwDevice], dwMode, bSimplerShadows );
        return TRUE;
    }
...
//-----------------------------------------------------------------------------
HRESULT CMyApplication::VideoOptions()
{
		BOOL bSimplerShadows = !bTestShadow;
		TEMPSTRUCT dialogData = { &m_pDeviceInfo, &bSimplerShadows };

 		// OutputToLog( "Prompting video options at user's request..." );

   if( IDOK == DialogBoxParam( (HINSTANCE)GetModuleHandle(NULL),
                                MAKEINTRESOURCE(IDD_CHANGEDEVICE),
                                GetForegroundWindow(),
                                ChangeDeviceProc, (LPARAM)&dialogData ) ){
			if( bTestShadow != ( !bSimplerShadows ) ){ 
				// must release shadow before call to change3denvironment()
				ReleaseShadowTexture();
				shadowTexture.pddsSurface = NULL;
				bTestShadow = !bSimplerShadows;
				if( !bTestShadow ){
...
// Desc: Pops up a dialog which allows the user to select a new device.
//-----------------------------------------------------------------------------
HRESULT CMyApplication::ForcedVideoOptions()
{
		BOOL bSimplerShadows = !bTestShadow;
		TEMPSTRUCT dialogData = { &m_pDeviceInfo, &bSimplerShadows };

		// OutputToLog( "Prompting video options..." );

    if( IDOK == DialogBoxParam( (HINSTANCE)GetModuleHandle(NULL),
                                MAKEINTRESOURCE(IDD_FORCEDCHANGEDEVICE),
                                GetForegroundWindow(),
                                ChangeDeviceProc, (LPARAM)&dialogData ) ){
			if( bTestShadow != ( !bSimplerShadows ) ){ 
				// must release shadow before call to change3denvironment()
...
//-----------------------------------------------------------------------------
HRESULT MyEnum_SelectDefaultDevice( D3DEnum_DeviceInfo** ppDevice,
                                     DWORD dwFlags )
{
    // Check arguments
    if( NULL == ppDevice )
...
*/
		...
		// avoiding TnL device because it appears to be more slow.
